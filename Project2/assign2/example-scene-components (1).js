  ...
  { 
      ...
      { 
        ...
        this.define_data_members( { purplePlastic : context.shaders_in_use["Phong_Model" ].material( Color( .90,.50,.9, 1 ), .4, .4, .8, 40 ),
                                    ...
                                    groundGreen	  : context.shaders_in_use["Phong_Model" ].material( Color(   0,  1, 0,.8 ), .4, .8, .4, 100),
    ...
    'draw_background'( model_transform, graphics_state )
      {
    	...
    	this.shapes.box.draw(graphics_state, mult(background_transform, mult(translation(0, -1, 0), scale(1000, 1, 1000) ) ) , this.groundTextile);
    	background_transform = mult(background_transform, rotation(180, [1,0,0]));
    	this.shapes.ball.draw(graphics_state, mult(background_transform, scale( 750, 750, 750 ) ), this.sky );
      },

    'draw_axis'(model_transform, graphics_state)
      {
    	this.shapes.box.draw(graphics_state, mult(model_transform, scale(10, 10, .01)), this.yellowGlass);
    	this.shapes.box.draw(graphics_state, mult(model_transform, scale(10, .01, 10)), this.redGlass);
    	this.shapes.box.draw(graphics_state, mult(model_transform, scale(.01, 10, 10)), this.blueGlass);
      },
    ...
    'display'( graphics_state )
      { 
      	var fp_transform = graphics_state.camera_transform;
      	var t_s = this.globals.graphics_state.animation_time/1000;
        ...
        this.draw_axis(model_transform, graphics_state);

        this.draw_gun(model_transform, graphics_state);
        ...
        {
        	this.bullets.push(mult(fp_transform, scale(5,5,5)));
        	this.bullet_make = false;
        }

        if(this.shot_fired == true && t_s < (this.shot_time + .25)) 
       	{
       		fp_transform = mult(fp_transform, rotation(-20*(this.shot_time + .25 - t_s), [1,0,0]));
       	...
       	{
       		this.bullets[i] = mult(this.bullets[i], translation(1,0,1));
       		this.shapes.ball.draw(graphics_state, mult(identity(), translation(i,0,0)), this.purplePlastic);
       	}
      }
  }, Scene_Component );  // End of class definition
...
Declare_Any_Class( "Example_Camera",                  // An example of a Scene_Component that our Canvas_Manager can manage.  Adds both first-person and
  { 'construct'( context, canvas = context.canvas )   // third-person style camera matrix controls to the canvas.
      ...
      // 1st parameter below is our starting camera matrix.  2nd is the projection:  The matrix that determines how depth is treated.  It projects 3D points onto a plane.
        ...
        camera_start = translation(0, -7, -50);
    ...
    'init_keys'( controls )   // init_keys():  Define any extra keyboard shortcuts here
      { 
      	controls.add( "Space", this, function() { this.thrust[1] =  0; } );     
        ...
        controls.add( "w",     this, function() { this.thrust[2] =  5; } );     
        controls.add( "w",     this, function() { this.thrust[2] =  0; }, {'type':'keyup'} );

        controls.add( "a",     this, function() { this.thrust[0] =  3; } );     
        controls.add( "a",     this, function() { this.thrust[0] =  0; }, {'type':'keyup'} );

        controls.add( "s",     this, function() { this.thrust[2] = -5; } );     
        controls.add( "s",     this, function() { this.thrust[2] =  0; }, {'type':'keyup'} );

        controls.add( "d",     this, function() { this.thrust[0] = -3; } );     
        ...
        controls.add( ".",     this, function() { this.graphics_state.camera_transform = mult( rotation( 6, 0, 0, -1 ), this.graphics_state.camera_transform ); } );
        controls.add( "Right", this, function() { this.graphics_state.camera_transform = mult( rotation( 2, 0, 1,  0 ), this.graphics_state.camera_transform ); } );
        controls.add( "Left",  this, function() { this.graphics_state.camera_transform = mult( rotation( 2, 0,-1,  0 ), this.graphics_state.camera_transform ); } );
